...
class HtmlType extends AbstractType
{
	...
	{
		parent::addAction();
		add_action('cmb2_render_html', 'FieldType\\Type\\HtmlType::render', 10, 5);
	}

	public static function render($field, $value, $objectId, $objectType, $fieldType)
	{
		// Parse args
		$attrs = $fieldType->parse_args('html', [
			'id' => $fieldType->_id(),
		]);

		if (isset($field->args['content_cb']) && !empty($field->args['content_cb'])) {
			ob_start();
			call_user_func_array($field->args['content_cb'], [$field, $objectId, $objectType]);
